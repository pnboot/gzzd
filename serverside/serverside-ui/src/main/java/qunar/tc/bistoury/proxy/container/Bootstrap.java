...
import static qunar.tc.bistoury.proxy.config.properties.GlobalConfig.serverMap;
    ...
    public static void main(String[] args) throws Exception {
        try {
           ...
           // DynamicConfig<LocalDynamicConfig> config = DynamicConfigLoader.load("server.properties");
            ...
            Conf config = Conf.fromMap(serverMap);
            int port = config.getInt("tomcat.port",8080);
            System.setProperty("bistoury.tomcat.port", String.valueOf(port));

            Tomcat tomcat = new Tomcat();
            tomcat.setPort(port);

            tomcat.setBaseDir(config.getString("tomcat.basedir","/tmp"));
